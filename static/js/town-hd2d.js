...
(function () {
    ...
    // Phase 5 (HD-2D): MegaKit 真 3D 拼裝整段已移除，7 棟全走 sprite billboard fallback

    // ─── Phase 6 (HD-2D): NPC 村長/商人 sprite billboard 在廣場 + 點擊互動
    const npcSpots = [
        { key: 'elder',    img: 'assets/images/world/scholar.png', x: -8, z:  3, size: 5,
          title: '村長伯伯', body: '歡迎來到 EduArena 學園村！<br>5 個科目場地等你來探索 ⚔️' },
        { key: 'merchant', img: 'assets/images/world/shopper.png', x:  8, z: -3, size: 4.5,
          title: '商人阿姨', body: '更衣室商店有新款上架<br>今天買滿 100 金送幸運寶箱 🎁' },
    ];
    npcSpots.forEach(npc => {
        const tex = textureLoader.load(npc.img);
        ...
        const sprite = new THREE.Sprite(spriteMat);
        sprite.scale.set(npc.size, npc.size, 1);
        sprite.position.set(npc.x, npc.size / 2, npc.z);
        scene.add(sprite);

        const hitbox = new THREE.Mesh(
            new THREE.BoxGeometry(npc.size * 0.7, npc.size, npc.size * 0.7),
            new THREE.MeshBasicMaterial({ visible: false })
        );
        hitbox.position.set(npc.x, npc.size / 2, npc.z);
        hitbox.userData.key = 'npc:' + npc.key;
        hitbox.userData.npcData = npc;
        scene.add(hitbox);
        buildingMeshes.push(hitbox);
        ...
        labelCanvas.width = 256; labelCanvas.height = 64;
        const ctx = labelCanvas.getContext("2d");
        ctx.fillStyle = "rgba(60,30,0,0.78)";
        roundRect(ctx, 0, 0, 256, 64, 12);
        ctx.fillStyle = "#ffe5b0";
        ctx.font = "bold 28px sans-serif";
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.fillText("💬 " + npc.title, 128, 32);
        const labelTexture = new THREE.CanvasTexture(labelCanvas);
        const labelMat = new THREE.SpriteMaterial({ map: labelTexture, depthTest: false });
        const label = new THREE.Sprite(labelMat);
        label.position.set(npc.x, npc.size + 1.5, npc.z);
    ...
    function openModal(key) {
        // NPC: 從 hitbox userData.npcData 拿對話資料
        if (key && key.startsWith('npc:')) {
            const hitbox = buildingMeshes.find(m => m.userData.key === key);
            const npc = hitbox && hitbox.userData.npcData;
            if (!npc) return;
            document.getElementById("demo-modal-icon").textContent = "💬";
            document.getElementById("demo-modal-title").textContent = npc.title;
            document.getElementById("demo-modal-body").innerHTML = npc.body;
    ...
    // Phase 6 (HD-2D): BokehPass DoF 景深模糊 — Octopath Traveler II 招牌電影感
    // focus 每 frame 對焦到玩家距離 → 玩家清晰、遠近景虛化
    const bokehPass = new BokehPass(scene, camera, {
        focus: 30.0,
        aperture: 0.00018,  // 光圈（越大模糊越強）
        maxblur: 0.009
    });
