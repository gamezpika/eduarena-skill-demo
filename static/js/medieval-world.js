...
(function () {
    ...
    // ─── Phase 5: 7 中央建築用 Medieval Village MegaKit (Quaternius CC0) 模組拼成
    ...
    BUILDINGS.forEach(b => {
        ...
        if (ISLAND_3D_KEYS.includes(b.key)) {
            ...
            scene.add(label);
        } else if (MEDIEVAL_KEYS.includes(b.key)) {
            // Phase 5: 7 中央建築先建 hitbox+label，房子在 buildVillage() async 階段拼
            const hitbox = new THREE.Mesh(
    ...
    // ─── Phase 5: MegaKit batch load + 7 棟中世紀房子拼裝 + props 散佈
    const MK_BASE = 'assets/3d/MedievalVillage_Standard/glTF/';
    const MK_PIECES = {};
    ...
    function loadMKPiece(name) {
        return new Promise(resolve => {
            megaLoader.load(MK_BASE + name + '.gltf', gltf => {
                gltf.scene.traverse(c => {
                    if (c.isMesh) { c.castShadow = true; c.receiveShadow = true; }
                });
                MK_PIECES[name] = gltf.scene;
                resolve();
            }, undefined, err => {
                console.warn('MegaKit load failed', name, err);
                resolve();
            });
        });
    }

    function cloneMK(name) {
        const tmpl = MK_PIECES[name];
        if (!tmpl) return null;
    ...
    const MK_WALL_W = 2;      // wall 寬 2m
    const MK_FLOOR_H = 3.12;  // 每層 3.12m

    function buildMedievalHouse(spec, cx, cz, rotY = 0) {
        const g = new THREE.Group();
        const n = spec.footprint;
        const size = n * MK_WALL_W;
        const half = size / 2;
        ...
        const sides = [
            { axis: 'x', perp: -half, rotY: 0           },  // south
            { axis: 'z', perp:  half, rotY: -Math.PI/2  },  // east
            { axis: 'x', perp:  half, rotY: Math.PI     },  // north
            { axis: 'z', perp: -half, rotY: Math.PI/2   },  // west
        ];

        for (let floor = 0; floor < spec.floors; floor++) {
            const yBase = floor * MK_FLOOR_H;
            for (let s = 0; s < 4; s++) {
                const side = sides[s];
                for (let i = 0; i < n; i++) {
                    ...
                    let px, pz;
                    if (side.axis === 'x') { px = t; pz = side.perp; }
                    else { px = side.perp; pz = t; }
            ...
            cornerSpots.forEach(([cxL, czL, cRotY]) => {
                const c = cloneMK(corner);
                if (!c) return;
                c.position.set(cxL, yBase, czL);
                c.rotation.y = cRotY;
                g.add(c);
            });
        }
        ...
        // 屋頂
        const totalH = spec.floors * MK_FLOOR_H;
        if (spec.roofStyle === 'tower') {
            const tower = cloneMK('Roof_Tower_RoundTiles');
            if (tower) {
                tower.position.set(0, totalH, 0);
                const ts = size / 4.5;
                tower.scale.set(ts, ts, ts);
                g.add(tower);
            }
        } else {
            // 4x4 RoundTiles 屋頂 (mesh footprint 5.51 x 5.56)
            const roof = cloneMK('Roof_RoundTiles_4x4');
            if (roof) {
                const rs = size / 5.51;
                roof.position.set(0, totalH, 0);
                roof.scale.set(rs, rs, 1.1 * rs);  // 深度方向略放大
                g.add(roof);
            }
        }
        ...
        // 爬藤裝飾（隨機 1-2 處）
        if (Math.random() < 0.7) {
            const v = cloneMK(Math.random() < 0.5 ? 'Prop_Vine1' : 'Prop_Vine4');
            if (v) {
                v.position.set(-half + 0.1, 0.1, -half + Math.random()*size*0.5);
    ...
    function scatterMedievalProps() {
        // 3 台 wagon 散在村莊內部空地
        const wagonSpots = [[-15, -22, 0.6], [10, 22, -0.8], [-12, 5, 1.5]];
        wagonSpots.forEach(([wx, wz, rot]) => {
            const w = cloneMK('Prop_Wagon');
            if (!w) return;
        ...
        // 木桶 + 磚塊散件
        const crateSpots = [[-28, -8], [-26, -8], [28, 12], [-15, 18], [32, -2], [-5, -28], [22, -18]];
        crateSpots.forEach(([cx, cz]) => {
            const c = cloneMK('Prop_Crate');
            if (!c) return;
            c.position.set(cx + (Math.random()-0.5)*2, 0, cz + (Math.random()-0.5)*2);
            c.rotation.y = Math.random() * Math.PI * 2;
            c.scale.set(1.2, 1.2, 1.2);
            scene.add(c);
        });

        const brickSpots = [[-25, -12], [27, -8], [-3, 27], [18, 18], [-18, 7]];
        brickSpots.forEach(([bx, bz]) => {
            const b = cloneMK(Math.random() < 0.5 ? 'Prop_Brick1' : 'Prop_Brick2');
            if (!b) return;
        ...
        // 木圍欄圈起來 — 在 5 個建築周圍各擺 4-5 段
        const fenceTargets = ['shop','farm','pvp','quest','museum'];
        fenceTargets.forEach(key => {
            const b = BUILDINGS.find(x => x.key === key);
            if (!b) return;
            const r = 7 + Math.random()*2;
            const segCount = 6;
            for (let i = 0; i < segCount; i++) {
                const a = (i / segCount) * Math.PI * 2 + Math.PI/6;
                const f = cloneMK('Prop_WoodenFence_Single');
                if (!f) continue;
                f.position.set(b.x + Math.cos(a)*r, 0, b.z + Math.sin(a)*r);
    ...
    Promise.all(MK_REQUIRED.map(loadMKPiece)).then(() => {
        // 拼 7 棟 — 房子朝向：south 牆面對地圖中心（門對中央廣場）
        BUILDINGS.forEach(b => {
            if (!MEDIEVAL_KEYS.includes(b.key)) return;
            const spec = HOUSE_SPECS[b.key];
            if (!spec) return;
            // rotY 讓 south 牆（door wall）朝向地圖中心 (0,0)
            const rotY = Math.atan2(0 - b.x, 0 - b.z) + Math.PI;
            const house = buildMedievalHouse(spec, b.x, b.z, rotY);
            house.scale.multiplyScalar(spec.scale || 1.0);
            scene.add(house);
        });
        scatterMedievalProps();
        console.log('[MegaKit] village built:', Object.keys(MK_PIECES).length, 'pieces loaded');
    });
